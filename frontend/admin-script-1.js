    ...
    try {
      ...
      document.getElementById('user-details').innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:.83rem;">
          ...
          <div><span style="color:var(--color-text-muted);">Status</span><div style="font-weight:600;color:${me.is_active?'var(--color-primary)':'var(--color-danger)'};margin-top:2px;">${me.is_active?'Active':'Inactive'}</div></div>
        </div>`;
    } catch { document.getElementById('user-details').innerHTML='<div class="empty-hint">Could not load user info.</div>'; }
  }

  async function loadAll() {
    await loadCurrentUser();
    await loadPlatformKPIs();
    await loadAnalytics();
