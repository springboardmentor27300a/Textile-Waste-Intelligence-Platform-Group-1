...
from routes import auth, admin, textile, ai, users
...
from routes.textile import get_current_user_from_token
...
def startup_db_client():
    ...
    try:
        ...
        admin_email = "madhulikagoddumarri@gmail.com"
        admin_password_hash = hash_password("123456789")
...
def create_inventory_alias(data: TextileRecordCreate, auth_data=Depends(get_current_user_from_token), db=Depends(get_db)):
    from routes.textile import create_textile_record
...
def list_inventory_alias(
    ...
    from routes.textile import list_textile_records
...
def get_inventory_alias(id: int, auth_data=Depends(get_current_user_from_token), db=Depends(get_db)):
    from routes.textile import get_textile_record
...
def update_inventory_alias(id: int, data: TextileRecordEdit, auth_data=Depends(get_current_user_from_token), db=Depends(get_db)):
    from routes.textile import update_textile_record
...
def delete_inventory_alias(id: int, auth_data=Depends(get_current_user_from_token), db=Depends(get_db)):
    from routes.textile import delete_textile_record
...
def dashboard_alias(auth_data=Depends(get_current_user_from_token), db=Depends(get_db)):
    from routes.textile import get_stats
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "service": "AI Textile Waste Intelligence Platform Backend API"}
